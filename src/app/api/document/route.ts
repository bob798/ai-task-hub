...
export async function POST(request: NextRequest) {
  ...
  if (text.trim().length > DOC_MAX_LENGTH) {
    return NextResponse.json(
      { error: `文本不能超过 ${DOC_MAX_LENGTH} 个字符` },
      { status: 400 }
    );
  }
  ...
  if (!apiKey) {
    return NextResponse.json({
      result: getMockResult(modeLabel, text.trim()),
      cost: DOC_PRICE,
      mock: true,
    });
  }
  ...
  try {
    ...
    return NextResponse.json({ result, cost: DOC_PRICE, mock: false });
  } catch (err) {
