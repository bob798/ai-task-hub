...
│       └── document/         # 文档处理 API（Route Handler）
├── components/               # Navbar / Footer
└── lib/
    ├── openai.ts             # OpenAI 客户端（图片 + Chat）
    ├── pricing.ts            # 定价表与费用计算
    ├── code.ts               # 代码生成语言列表
    ├── document.ts           # 文档处理模式定义
    ├── wallet.ts             # 本地钱包：余额、充值、扣费（localStorage）
...
- [x] 代码生成（GPT-4o mini）
- [x] 文档处理（总结 / 翻译 / 分析）
- [x] 钱包与余额系统（充值 / 扣费 / 收支明细，客户端实现）
- [ ] 服务端账户与真实支付接入（需数据库与支付渠道）
